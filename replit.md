...
- **api-server**: Express 5 API. Hosts the Clerk proxy at `CLERK_PROXY_PATH` (mounted before body parsers) and `clerkMiddleware()` for verifying sessions. Mounts `/api/payments/*` (NOWPayments proxy: `POST /create`, `GET /:id/status`, `GET /health`) — uses `NOWPAYMENTS_API_KEY` secret, `NOWPAYMENTS_API_BASE` env override (defaults to `https://api.nowpayments.io/v1`). Supported pay currencies: `btc`, `eth`, `sol`, `usdttrc20`, `usdterc20`. Frontend `useTradingBot.createDeposit` posts to `/api/payments/create` and polls `/status` every 8s; on `confirmed`/`finished` it credits the local USD balance. Withdrawals remain local-only. Also mounts `/api/bot/*` (`POST /upload`, `GET /health`) which validates the administrator-issued bot pass key (env var `BOT_UNLOCK_KEY`, currently `AT6768665G`) using constant-time comparison; the key never appears in the frontend bundle. Frontend `useTradingBot.unlockBot` calls `/api/bot/upload` and only sets `unlocked: true` on a `200 {ok:true}` response.
