...
- **horizon-markets** (`/`): Crypto trading landing page for "Horizon Markets". React + Vite + Tailwind v4 + framer-motion + recharts. Dark theme with neon green (`#00FF88`) accents, Inter font. Sections: Navbar, Hero, Ticker, MarketOverview, Features, Stats, MobileApp, PriceTable, FinalCTA, Footer. Hero/phone/blockchain images generated via media-generation.
...
- **Frontend** (`horizon-markets/src/App.tsx`): `ClerkProvider` with `dark` base theme + custom appearance (neon green primary, dark surfaces, custom Horizon logo). Routes: `/sign-in/*?`, `/sign-up/*?`, `/dashboard`. `HomeRedirect` sends signed-in users from `/` to `/dashboard`. `DashboardGuard` enforces auth on `/dashboard`. Wouter `<Router base={basePath}>` + a `routerPush`/`routerReplace` that strips basePath for Clerk navigation.
