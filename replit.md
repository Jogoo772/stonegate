...
- **horizon-markets** (`/`): Crypto trading landing page for "Stonegate" (artifact directory still named `horizon-markets` for stability). React + Vite + Tailwind v4 + framer-motion + recharts. Dark theme with neon green (`#00FF88`) accents, Inter font. Sections: Navbar, Hero, Ticker, MarketOverview, Features, Stats, MobileApp, PriceTable, FinalCTA, Footer. Hero/phone/blockchain images generated via media-generation.
...
Mounts `/api/credits/*` for admin-issued manual balance top-ups: user routes `GET /pending?userId=...` (returns un-applied PENDING credits) and `POST /:id/ack` (user acknowledges they've applied it; second ack returns 409 to keep the apply race-safe). Admin routes (`x-admin-key`): `GET /admin` (list all), `POST /admin` (create credit; body `{userId, amount, note?}`), `POST /admin/:id/cancel` (cancel a still-pending credit). Persistence: `artifacts/api-server/data/admin-credits.json` via `lib/creditsStore.ts` (same atomic-write chain pattern as withdrawals). Credit status: `PENDING | APPLIED | CANCELLED`. Frontend `useTradingBot` polls `/credits/pending` every 10s, acks each new credit (only the device that wins the ack — server returns 409 to losers), and on success adds the amount to local USD balance. Admin console gained a tabbed UI: "Withdrawals" tab (existing review queue) and "Manual credit" tab (form to enter Clerk userId + USD amount + optional note, plus a recent-credits list with cancel button for still-pending entries). Manual credits also raise the bot's 3× minimum-balance threshold, just like deposits — `useTradingBot` tracks `creditedTotal` (incremented on every successful ack, persisted in localStorage) and `depositMetrics` now computes `requiredMinimumBalance = (totalDeposited + totalCredited) * 3`.
...
- **Frontend** (`horizon-markets/src/App.tsx`): `ClerkProvider` with `dark` base theme + custom appearance (neon green primary, dark surfaces, custom Stonegate logo). Routes: `/sign-in/*?`, `/sign-up/*?`, `/dashboard`. `HomeRedirect` sends signed-in users from `/` to `/dashboard`. `DashboardGuard` enforces auth on `/dashboard`. Wouter `<Router base={basePath}>` + a `routerPush`/`routerReplace` that strips basePath for Clerk navigation.
